...
# The third breakthrough was self-attention: dynamic, input-dependent connections that let
# each token mix information from other tokens. Depth and width are fixed capacity changes;
# attention adapts connections per input sequence.
...
# softmax(M) is uniform over allowed past tokens.
# softmax(B) is recency-biased, so attention decays with distance.
# B also masks future positions, so M is redundant.
...
# 1. 10B params -> ~200B tokens (20x).
# 2. 2T tokens -> ~100B params (divide by 20).
# 3. Early LLMs were over-parameterized and under-trained (too few tokens per parameter).
...
#    possible completions  can quickly become intractable.
...
# False. In traditional text processing pipelines, removal of
...
# and better performing option is ALiBi, which adds position-dependent linear biases 
...
#    incorrect label  causesthe model to produce wrong outputs. 
#    The model mimics the pattern of the examples, even if the labels are incorrect.
#
...
# Word-based: large vocabularies, slow training/inference.
...
#
# Subword tokenization  balances the size of vocabularies, the number of
# out-of-vocabulary tokens, and the length of sequences. Frequently used words stay
# whole; rare words are split into meaningful subparts..
...
#RAG  reduces hallucinations by grounding the model's
...
# the other time steps that were used as input.
#
